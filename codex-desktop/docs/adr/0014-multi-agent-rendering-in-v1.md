...
**Update:** implemented. Both item types now render per-tool text following `tui/src/multi_agents.rs::tool_call_history_cell` — including its rule that in-flight `spawnAgent`/`sendInput`/`closeAgent` calls render *nothing* and only `wait`/`resumeAgent` narrate while running, which is what keeps a burst of collab calls from flooding the stream. `agentsStates` renders as the per-agent status map it is. Navigation into an agent's own thread is ADR-0023.
